...
def main():
    ...
    print("Got {} encoder embeddings from {}".format(
        len(src_vectors), opt.emb_file_enc))

    tgt_vectors = read_embeddings(opt.emb_file_dec)
    print("Got {} decoder embeddings from {}".format(
    ...
    print("\t* enc: %d match, %d missing, (%.2f%%)"
                % (enc_count['match'],
                   enc_count['miss'],
                   match_percent[0]))
    print("\t* dec: %d match, %d missing, (%.2f%%)"
                % (dec_count['match'],
                   dec_count['miss'],
                   match_percent[1]))

    print("\nFiltered embeddings:")
    print("\t* enc: %s" % str(filtered_enc_embeddings.size()))
    print("\t* dec: %s" % str(filtered_dec_embeddings.size()))
